...
export function loadStoredUserWithExpiry<T extends { sessionExpiry?: number }>(
  maxAgeMs?: number
): T | null {
  const parsed = getStoredUser<T>();
  ...
  if (maxAgeMs && parsed.sessionExpiry && Date.now() >= parsed.sessionExpiry) {
    clearAuthSession();
    return null;
  }
